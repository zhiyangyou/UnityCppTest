	...
	public static class GenerateBindings
	{
		...
		private static string GitBaseDir
		{
			get
			{
				...
				{
					_gitBaseDir = new DirectoryInfo(Application.dataPath).Parent.Parent.FullName;
				}
		...
		private static string CsharpPath  => $@"{GitBaseDir}\UnityCppTest\Assets\Scripts\App\CppPlugin\UnityBindings\UnityBindings.cs";
		...
		private static readonly string jsonPath = @$"{GitBaseDir}\UnityCppTest\Assets\Scripts\AppEditor\NativeScriptTypes.json";
		...
		static Assembly[] GetAssemblies(string[] assemblyNames)
		{
			...
			assemblies[32] = typeof(UnityEngine.Networking.UnityWebRequestAsyncOperation).Assembly; // Unity network module
#else
